...
namespace Triggerless.TriggerBot
{
    public partial class AddnTriggersForm : Form
    {
        ...
        private bool HasGridChanged()
        {
            ...
            {
                if (gridTriggers.Rows[iRow].Cells[COL_ADDN_TRIGGERS].Value.ToString() !=
        ...
        private void SaveChanges()
        {
            ...
            using (var conn = sda.GetAppCacheCxn())
            {
                ...
                for (int iRow = 0; iRow < gridTriggers.Rows.Count; iRow++)
                {
                    var cellValue = gridTriggers.Rows[iRow].Cells[COL_ADDN_TRIGGERS].Value.ToString();
                    if (cellValue != Product.Triggers[iRow].AddnTriggers)
                    {
                        ...
                        var sql = $"UPDATE product_triggers SET addn_triggers = {value} WHERE product_id = {trigger.ProductId} ABD prefix = '{trigger.Prefix}' AND sequence = {trigger.Sequence}";
                        conn.Execute(sql);
                    }
                }
            }
